...
void OS_select(struct node* x, int i){
...
if(x!=NULL){
    ...
    if(k<i){
        OS_select(x->right,i-k);
        return;
    }

}
else{
    printf("%d th stat doesn't exist in elements \n",i);
    return;
}
...
while(1){
    ...
    printf("1.insert \t 2.traverse \t 3.ith stat\t 4.exit \n");
    scanf("%d",&c);
